...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	img_copy = img.copy()
	all_img = img_copy.flatten()
	np.random.shuffle(all_img)

	"*** END YOUR CODE HERE ***"
	# reshape the shuffled image
	shuffled_img = all_img.reshape(img.shape)
	...
	U_sf, S_sf, V_sf = np.linalg.svd(shuffled_img)
	...
	"*** YOUR CODE HERE ***"
	orig_plot, = plt.plot(S[:k], 'b')
	shuffled_plot, = plt.plot(S_sf[:k], 'r')
