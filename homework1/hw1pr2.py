"""
Start file for hw1pr2 of Big Data Spring 2019
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	X_space = np.linspace(0.0, 5.0, num=100)
	X_space = X_space.reshape((-1,1))
	y_space = m_opt * X_space + np.full(100, b_opt).reshape((-1,1))
	...
	"*** YOUR CODE HERE ***"
	d = np.arange(100)
	X_space_stacked = np.hstack((np.ones_like(d).reshape((-1,1)), X_space)) 
	a = np.matmul(X_space_stacked.T, X_space_stacked)
	b = np.matmul(X_space_stacked.T, y_space_rand)
