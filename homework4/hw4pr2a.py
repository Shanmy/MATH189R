...
def grad_logreg(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	grad = np.dot(X.T, (sigmoid(np.dot(X, W)) - y)) + reg * W
...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	u = sigmoid(np.dot(X, W))
	temp = np.multiply(y, np.log(u)) + np.multiply((1. - y), np.log(1. - u))
	nllAll = -sum(temp) + reg / 2 * np.linalg.norm(W) ** 2
	nll = nllAll.item(0)
...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		# Calculate the negative log likelihood at each iteration and
		# append the value to nll_list
		...
		# Calculate the gradient for W and upgrade W
		W = W - lr * grad_logreg(X, y, W, reg = reg)
		"*** END YOUR CODE HERE ***"
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0., 0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 15.0, 20.0, 30.0]
