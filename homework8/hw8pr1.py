...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			point = X[i, :]
			diff = point - clusters
			order = np.argsort(np.linalg.norm(diff, axis=1)).item(0)
			label[i] = order
		# update centers
		for i in range(k):
			points_in_cluster = np.where(label == i)[0]
			if len(points_in_cluster) > 0:
				clusters[i, :] = X[points_in_cluster].mean(axis=0)
		"*** END YOUR CODE HERE ***"
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	ost_k_plot, = plt.plot(range(1, 21), cost_k_list, 'g^')
